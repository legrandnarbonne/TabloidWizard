...
namespace TabloidWizard
{
    ...
    public class MRUManager
    {
        ...
        public void Initialize(Form owner, ToolStripMenuItem mruItem, string regPath)
        {
            ...
            if (registryPath.EndsWith("\\",StringComparison.InvariantCulture))
                registryPath += "MRU";
            else
                registryPath += "\\MRU";
        ...
        private void OnMRUParentPopup(object sender, EventArgs e)
        {
            ...
            while (myEnumerator.MoveNext())
            {
                var fileName = (string)myEnumerator.Current;
        ...
        private void OnOwnerClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                if (key != null)
                {
                    ...
                    {
                        key.DeleteValue(regEntryName + i, false);
                    }

                    for (i = 0; i < n; i++)
                    {
                        key.SetValue(regEntryName + i, mruList[i]);
                    }
                }
            }
            catch (Exception ex)
